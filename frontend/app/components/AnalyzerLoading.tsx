...
export default function AnalyzerLoading({ repoUrl, onFinished }: AnalyzerLoadingProps) {
  ...
  return (
    <div className="flex-grow flex flex-col items-center justify-center p-6 max-w-2xl mx-auto w-full font-sans">
      {/* Monospaced Loading Indicator */}
      <div className="text-center mb-8">
        <div className="text-accent text-sm font-extrabold uppercase tracking-widest animate-pulse font-mono">
          [ SCANNING_CODEBASE ]
        </div>
        <div className="text-[10px] text-muted mt-2 select-none truncate max-w-full font-mono">
          {repoUrl}
        </div>
      </div>

      {/* Progress Bar (Flat) */}
      <div className="w-full bg-panel border border-border rounded-lg h-2.5 mb-8 overflow-hidden">
        <div
